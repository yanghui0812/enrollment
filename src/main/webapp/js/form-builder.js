  ...
  var FormBuilder = function FormBuilder(options, element) {
    ...
    var advFields = function advFields(values) {
     ...
     /* var accessLabels = { first: opts.messages.roles, second: opts.messages.limitRole, content: availableRoles.join('') };
      ...
      advFields.push(limitApplicantField(values));
    ...
    var limitApplicantField = function limitApplicantField(values) {
    	var noMake = [], appslot = '';
        if (values.type != 'select') {
          noMake.push(true);
        }
        if (!noMake.some(function (elem) {
          return elem === true;
           ...
           appslot = boolAttribute('slot', values, { first: opts.messages.appslot, second: opts.messages.appmaxsize});
        }
